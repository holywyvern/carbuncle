...
{
  mrb_int ox, oy, tx, ty, w, h, aw, ah, mx, my;
  Rectangle src_rect;
  struct mrb_Plane *plane = get_plane(mrb, self);
  if (!plane->texture)
  {
  ...
  aw = fabs(w);
  ah = fabs(h);
  ox = ruby_mod(plane->origin->x, aw);
  oy = ruby_mod(plane->origin->y, ah);
  tx = GetScreenWidth() / aw + 2;
  ty = GetScreenHeight() / ah + 2;
  mx = w < 0 ? -1 : 1;
  my = h < 0 ? -1 : 1;
  Vector2 origin = (Vector2){0, 0};
  src_rect = *plane->src_rect;
  src_rect.width *= mx;
  src_rect.height *= my;
  ...
  for (mrb_int i = -1; i < tx; ++i) 
  {
    ...
    {
      Rectangle dst_rect = {
        ox + aw * i, oy + ah * j, aw, ah
      };
      mrb_carbuncle_sprite_draw_texture(
        *(plane->texture), src_rect, dst_rect, origin, 0, *(plane->color)
      );
    }
  }
  rlDisableTexture();
